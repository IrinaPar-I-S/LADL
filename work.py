...
prepositions = {'cercare':'di','provare':'a','dimenticare':'di','sperare':'di','dire':'di','raccontare':'di','chiedere':'di',\
    'domandare':'di','promettere':'di','riferire':'a','dispiacere':'di','paura':'di','temere':'di','preoccuparsi':'di',\
    'cominciare':'a','iniziare':'a','continuare':'a','finire':'di','smettere':'di','concludere':'di','chiedere':'di',\
    'richiedere':'di','pregare':'di','ordinare':'di','indurre':'a','credere':'di','pensare':'di','dubitare':'di',\
    'rinunciare':'a','fingere':'di','riuscire':'a','cercare':'di','provare':'a','osare':'a','ricordare':'di','tentare':'di'}
...
def zero (lemma) :
    ...
    for page in range (1,1345) :
        response = requests.get('https://corpora.dipintra.it/public/run.cgi/view?q=aword%2C%5Blemma%3D"'+lemma+'"%5D&q=P-3+3+1+%5Bword%3D"questo"%7Cword%3D"quello"%7Cword%3D"ciò"%7Cword%3D"cio%27"%7Cword%3D"lo"%7Cword%3D"l%27"%5D;fromp='+str(page)+';corpname=itwac_full&viewmode=sen&refs=%23&lemma='+lemma)
...
def second (filename, questo, quello, lo, cio, verbasfilename) :
...
def forth () :
    verbs = ['potere','volere','dovere','bisogna','sentire','capire','sapere','realizzare','dimenticare','vedere','desiderare','sperare',\
    'confidare','dire','raccontare','informare','chiedere','domandare','promettere','riferire','annunciare','immaginare','dispiacere',\
    'paura','temere','preoccuparsi','cominciare','iniziare','continuare','finire','smettere','concludere','chiedere','richiedere',\
    'pregare','ordinare','indurre','sembrare','credere','pensare','dubitare','rinunciare','supporre','fingere','riuscire','cercare',\
    'provare','osare','ricordare','tentare']
...
def fifth (path, filename) :
    ...
    counter.extend([counter[0]*100/counter[4],counter[1]*100/counter[4],counter[2]*100/counter[4],counter[3]*100/counter[4]])
    output = [verb]
    output.extend(counter)
    with open ('res\output.csv', 'a') as file :
...
def sixth () :
    foldername = r'.\res'
    files = os.listdir(foldername)
    with open (r'res\output.csv', 'w') as file :
